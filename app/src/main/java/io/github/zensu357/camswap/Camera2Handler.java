...
public class Camera2Handler implements ICameraHandler {
    ...
    // ================================================================
        ...
        // ---- API 28+ Executor-based capture hooks (Used by modern CameraX) ----
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.P) {
            try {
                java.lang.reflect.Method captureSingle = resolveMethod(
                    classLoader,
                    "android.hardware.camera2.impl.CameraCaptureSessionImpl",
                    ...
                    .intercept(chain -> {
                        ...
                        if (sessionObj instanceof android.hardware.camera2.CameraCaptureSession
                                ...
                                android.hardware.camera2.CaptureRequest request =
                                    (android.hardware.camera2.CaptureRequest) args[0];
                                android.hardware.camera2.CameraCaptureSession.CaptureCallback callback =
                                    (android.hardware.camera2.CameraCaptureSession.CaptureCallback) args[2];
                                // Pass null for Handler; CaptureGate handles null by running directly
                                args[2] = io.github.zensu357.camswap.CaptureGate.wrapCallback(
                                    session, request, callback, null
                                );
                            }
                        } catch (Throwable t) {
                            io.github.zensu357.camswap.utils.LogUtil.log(
                                "【CS】【CaptureGate】captureSingleRequest hook error: " + t
                            );
                        }
                        return chain.proceed(args);
                    });
            } catch (Throwable t) {
                // captureSingleRequest may not exist on some heavily modified custom ROMs
            }

            try {
                java.lang.reflect.Method captureBurstReqs = resolveMethod(
                    classLoader,
                    "android.hardware.camera2.impl.CameraCaptureSessionImpl",
                    ...
                    .intercept(chain -> {
                        ...
                        try {
                            ...
                            if (sessionObj instanceof android.hardware.camera2.CameraCaptureSession
                                ...
                                android.hardware.camera2.CameraCaptureSession.CaptureCallback callback =
                                    (android.hardware.camera2.CameraCaptureSession.CaptureCallback) args[2];
                                args[2] = io.github.zensu357.camswap.CaptureGate.wrapBurstCallback(
                                    session, requests, callback, null
                                );
                            }
                        } catch (Throwable t) {
                            io.github.zensu357.camswap.utils.LogUtil.log(
                                "【CS】【CaptureGate】captureBurstRequests hook error: " + t
                            );
                        }
                        return chain.proceed(args);
                    });
            } catch (Throwable t) {
                // captureBurstRequests may not exist on some heavily modified custom ROMs
            }
        }
